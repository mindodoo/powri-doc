...
> **Phase 2:** Read **[`../phase2/architecture-phase2.md`](../phase2/architecture-phase2.md)** for Supabase, UGC, maps, passport, and trips. SQL migration: [`supabase/migrations/001_phase2_core.sql`](../../../../supabase/migrations/001_phase2_core.sql).

**Companion artifacts:**
- PRD v1.1 (`_powri/planning-artifacts/prds/phase1/prd-phase1.md`)
- UX spines (`ux-designs/ux-phase1/design.md`, `experience.md`)
- Epics (`epics/phase1/epics-phase1.md` — Phase 1 breakdown)
...
- **Phase 2 (see [`../phase2/architecture-phase2.md`](../phase2/architecture-phase2.md)):** Supabase auth + PostgreSQL, Google Places proxy, Leaflet surroundings map, Plan tab trip skeleton, reviews/passport/saved
...
**Moved to Phase 2** — see [`../phase2/architecture-phase2.md`](../phase2/architecture-phase2.md):
...
**Validation:** Zod schema mirroring `content-model.md` §4 + PRD §8.1. Dev warnings for unpublished slugs; build fails only if zero published resorts.
...
**Phase 2 auth:** Supabase Auth with HTTP-only cookies via `@supabase/ssr` — see [`../phase2/architecture-phase2.md`](../phase2/architecture-phase2.md) §4.
...
**Quiz (Phase 1):** Scoring via `src/lib/quiz/score.ts` per [`features/quiz/scoring.md`](../../features/quiz/scoring.md). UI: `QuizModeToggle`, `ResortQuizCard`, **`QuizResultsReveal`**, **`QuizTopMatchCard`**, **`QuizRunnerUpRow`**, `getMatchReason()` — Serious header only; Cosmic interstitial ~800ms + header + subline; **podium top 3** on results (`features/quiz/brief.md` § Results reveal + § Results podium). Shared enums for Serious/Cosmic; `quiz_completed` fires after reveal visible (`result_count: 3`).
...
2. Analytics event names (must match `docs/analytics/tracking-plan.md` exactly)
3. API error shape
...
- Read `architecture.md`, `design.md`, `experience.md`, and `docs/analytics/tracking-plan.md` before implementing a story
...
- **Phase 2:** [`../phase2/architecture-phase2.md`](../phase2/architecture-phase2.md) — Supabase, UGC, maps, trips (2026-06-23)
...
4. Instrument analytics per `docs/analytics/tracking-plan.md` in same PR as feature
